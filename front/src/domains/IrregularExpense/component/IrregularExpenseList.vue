...
import { useCategoryStore } from '@/domains/Category/store/categoryStore';
import { findSelectItemInCategories, mapCategoriesToSelectData } from '@/domains/Category/data/service/CategorySelectItem';
import { Categories } from '@/domains/Category/data/model/Category';
import { SubCategories } from '@/domains/Category/data/model/SubCategory';

const emit = defineEmits(['loadingError']);

const { getCategories, getFlatCategories } = useCategoryStore();
const categories = ref<Categories>();
const flatCategories = ref<SubCategories>();
try {
    categories.value = (await getCategories()).value;
    flatCategories.value = (await getFlatCategories()).value;
} catch (e) {
    emit('loadingError');
}
...
const categoriesSelect: VSelectData = [{
    ...
    subItems: [],
}, ...mapCategoriesToSelectData(categories.value as Categories)];
...
const hasAnythingBeenChanged = computed(() => {
    ...
    console.log(originalState.value, currentExpensesState)
                            ...
                            <div class="mt-2 flex flex-col gap-2">
                                <div>
                                    <VSelect 
                                        ...
                                        @changed="(category: VSelectItem) => expenseDv.expense.category = findSelectItemInCategories(category, flatCategories as SubCategories)"/>
                                    <div class="text-error font-medium mt-2">
            ...
            <div class="collapse-content space-y-3"> 
                ...
                <form @submit.prevent="addIrregularExpense()">
                    ...
                    <label class="flex gap-1 items-center">
                        ...
                        <VSelect 
                            :elements="categoriesSelect" 
                            @changed="(category: VSelectItem) => newExpense.category = findSelectItemInCategories(category, flatCategories as SubCategories)"/>
                    </label>
                    <label>
