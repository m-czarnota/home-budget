...
import { ComputedRef, PropType, Ref, computed, reactive, ref } from 'vue';
...
const props = defineProps({
    passedBudget: {
        type: Object as PropType<Budget|undefined>,
        required: false,
    },
})
...
let budget: Budget|undefined = props.passedBudget ? reactive(props.passedBudget) : undefined;

// ------------------- viewing categories and subcategories -------------------
const isNoBudget = computed(() => {
    return budget?.entries.length === 0;
});
...
const originalState = ref(JSON.stringify(budget));
const hasAnythingBeenChanged = computed(() => {
    const stringifiedBudget = JSON.stringify(budget);
...
const submit = async () => {
    try {
        ...
        budget = reactive(await BudgetService.update(budget as Budget));
        originalState.value = JSON.stringify(budget);
...
<template>
    <form @submit.prevent="submit" v-if="budget">
        <Alert 
            v-if="showSuccessAlert" 
            @closed="showSuccessAlert = false"
            :message="$t('component.prepareBudget.successSave')"/>

        <Alert
            v-if="showErrorAlert"
            type="danger"
            @closed="showErrorAlert = false"
            :message="$t('component.prepareBudget.errorSave')"/>
        ...
        <p v-if="hasAnythingBeenChanged" class="bg-red-200 rounded p-2" role="alert">
            ...
            <span>{{ $t('component.prepareBudget.unsavedChanges') }}</span>
        </p>
        ...
        <h2 class="text-xl">
            {{ $t('component.prepareBudget.budgetForMonth') }}
            {{ getMonthNameForNumber(budget?.month || 0) }}
        </h2>
        ...
        <div v-for="(budgetEntry, entryIndex) in budget?.entries || []" class="space-y-2">
            <div class="flex">
            ...
            <div v-if="hasEntryChildren(budgetEntry)" class="space-y-2">
                <div v-for="(subEntry, subEntryIndex) in budgetEntry.subEntries" class="flex *:pl-4">
                    <div class="flex-1">{{ subEntry.categoryName }}</div>
                    <div class="flex-1">
        ...
        <button type="submit" :disabled="!hasAnythingBeenChanged || isSyncPending" class="flex items-center justify-center gap-2">
            {{ $t('component.prepareBudget.save') }}
