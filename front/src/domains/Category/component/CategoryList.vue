...
const { getCategories, updateCategories } = useCategoryStore();

let categoriesDv: CategoryDataView[] = [];

try {
    const categoriesFromResponse = await getCategories();
    categoriesDv = reactive(categoriesFromResponse.value.map(category => new CategoryDataView(category))) as CategoryDataView[];
} catch (e) {
    emit('loadingError');
}
